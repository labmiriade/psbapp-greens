...
def put_places(csvfile, batch) -> Tuple[Set[str], Set[str], List[Exception]]:
    ...
    for row in reader:
        ...
        try:
            batch.put_item(
                Item={
                    ...
                    "data": {
                        ...
                        "name": get_or_error(row, "Nome_area"),
                        "category": get_or_error(row, "Classificazione").strip().lower(),
                        "lat": get_or_error(row, "Latitudine"),
                        "istatCode": get_or_error(row, "COD_ISTAT_Comune"),
                        "area": get_or_error(row, "Area_mq"),
                        "searchable": True,
                    },
                    "gsi1pk": "place",
                }
            ...
            ids.add(place_id)
            if row["classificazione"]:
                categories.add(row["classificazione"].strip().lower())
        except Exception as error:
            errors.append(error)
