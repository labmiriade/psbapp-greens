...
export class CoreConstruct extends Construct {
  constructor(scope: Construct, id: string, props: CoreConstructProps) {
    ...
    const greensImportLambda = new lambda.Function(this, 'GreensImportFn', {
      code: new lambda.AssetCode('../etl/import-greens', {
        bundling: {
      ...
      memorySize: 512,
      description: 'Lambda per importare aree verdi territoriali',
      environment: {
        DATA_TABLE: dataTable.tableName,
        CSV_DATA_URLS: props.csvDataUrls,
      },
    });
    dataTable.grantReadWriteData(greensImportLambda);

    const greensImportErrors = greensImportLambda.metricErrors({
      period: cdk.Duration.minutes(1),
    });

    const importAlarm = new cloudwatch.Alarm(this, 'greensImportErrorsAlarm', {
      metric: greensImportErrors,
      threshold: 1,
      comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
      evaluationPeriods: 1,
      alarmDescription: 'An error occurred during the GreensImport function execution',
    });
    ...
    eventRule.addTarget(new targets.LambdaFunction(greensImportLambda));
