    ...
    this.sub.add(
      this.action$.pipe(ofType(searchFailedById)).subscribe((props) => {
        if (props.placeId === this.placeId) {
          this.toast.show('Errore', "L'area cercata non esiste", ToastType.Danger);
    ...
    return this.place$?.pipe(
      map((place) => {
        ...
        if (place?.city) {
          ...
          let istatCode: string = place.istatCode ? ' ' + place.istatCode : '';
          ret = place.city + istatCode + province;
        }
        return ret;
      }),
    );
